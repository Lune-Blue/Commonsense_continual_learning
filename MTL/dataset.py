...
class MultiTaskDataset(Dataset):
    def __init__(self, tokenizer, multitask_list, total_len):
        ...
        for x,y,task_name in multitask_list:
            if task_name=="siqa":
                for siqa_point in x:
                    input_answers = [siqa_point[2], siqa_point[3], siqa_point[4]]
                    num_choices = len(input_answers)
                    input_context_question = [siqa_point[0] + self.roberta_tokenizer.sep_token + siqa_point[1]] * num_choices
                    encoded_text_train = self.roberta_tokenizer(input_context_question, input_answers, padding=True, return_tensors='pt')
                    self.x_tokenized.append(encoded_text_train)
                ...
                for point in x:
                    ...
                    self.task_name.append(task_name)
            for point in y:
                self.y_flatten.append(point)

    def __getitem__(self, idx):
        return (self.x_tokenized[idx], self.y_flatten[idx], self.task_name[idx])
...
def prepare_batch_MT(batch):
    ...
    features, labels, task_name = zip(*batch)
